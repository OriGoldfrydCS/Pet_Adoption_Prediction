"""
Saving utility for XGBoost evaluation results.

This function:
- Saves the trained model.
- Stores model hyperparameters, classification report, accuracy, F1, and log loss.
- Saves label distributions and a confusion matrix plot.
- Saves probabilities, ROC curves, and PR curves.

Called after evaluation on validation and test set.
"""
...
def save_run_outputs(model, X, y_true, y_pred, name, base_output_dir, use_timestamp_subfolder=True):
    """
    Saves evaluation results, plots, and model.

    Parameters:
    - model: Trained classifier (must support `.predict()` and optionally `.predict_proba()`).
    - X: Input features used for prediction (needed for ROC/log loss).
    - y_true: Ground truth labels.
    - y_pred: Model predictions.
    - name (str): Identifier for the dataset split ('val' or 'test'), used in filenames.
    - base_output_dir (str): Directory where results will be saved.
    - use_timestamp_subfolder (bool): Whether to create a timestamped subfolder in `base_output_dir`.
    ...
    # Attempt to compute log loss (if model supports predict_proba)
    logloss = None
    ...
    # Save class distributions (helpful for imbalance analysis)
    ...
    # Plot ROC & PR curves if predict_proba is supported
    if hasattr(model, "predict_proba"):
        try:
            y_proba = model.predict_proba(X)
