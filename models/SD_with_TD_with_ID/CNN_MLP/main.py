"""
Train and evaluate a CNN + MLP model using:
- Mean image per sample
- TF-IDF encoded text
- Structured data

This script loads the data, trains the model on train+val,
evaluates on both validation and test sets, and saves results.
"""
...
from models.SD_with_TD_with_ID.CNN_MLP.model_trainer import train_and_evaluate
from models.SD_with_TD_with_ID.CNN_MLP.evaluator import evaluate_model
...
def main():
    ...
    # Transpose images from [B, H, W, C] to [B, C, H, W] for CNN
    ...
    # Print input shapes and check for issues
    print(f"\nTrain shapes:")
    ...
    # Train model
    print("\nTraining CNN + MLP on image + TF-IDF text + structured features...")
    start = time()
    ...
    base_output_dir = os.path.join("performance_and_models", "cnn_mlp_text_image_structured", run_id)
    ...
    # Evaluate on test set (no precomputed predictions)
