"""
Evaluation and output saving utilities for multimodal classification models
(CNN for images + MLP for TF-IDF text + MLP for structured data).

This script includes:
- compute_crossentropy_loss: Computes average loss over a dataset.
- save_run_outputs: Saves model state, predictions, metrics, and visualizations (confusion matrix).
"""
...
def compute_crossentropy_loss(model, X_img, X_text, X_struct, y_true, batch_size=32):
    """
    Computes average cross-entropy loss on a given dataset (batched).
    """
...
def save_run_outputs(*, model, X_img, X_text, X_struct, y_true, y_pred, name, base_output_dir, use_timestamp_subfolder=True):
    """
    Saves all relevant evaluation outputs to a directory:
    - model.pt: trained model state
    - hyperparameters.txt: input shapes and architecture summary
    - true/pred labels: {name}_true.csv, {name}_pred.csv
