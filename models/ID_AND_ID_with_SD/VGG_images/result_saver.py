...
1. Resulting a VGG trained on **image data only**
2. Resulting a VGG trained on **image data + structured features**
...
# === Resulting VGG19 on image data only ===
...
def save_run_outputs(model, X, y_true, y_pred, name, base_output_dir, use_timestamp_subfolder=True, logs=None):
    ...
    try:
        ...
        if y_proba is not None:
            print("Raw y_proba type:", type(y_proba))
            print("Raw y_proba[0] type:", type(y_proba[0]))
            print("Raw y_proba[0] shape:", getattr(y_proba[0], 'shape', 'N/A'))

            y_proba = np.array(y_proba)

            if y_proba.dtype == np.object_:
                print("Detected object array. Attempting to stack...")
            ...
            if y_proba.shape[1] == len(np.unique(y_true)):
                ...
                print(f"Skipping log loss: shape mismatch — got {y_proba.shape[1]} classes, expected {len(np.unique(y_true))}")
    except Exception as e:
        print(f"Skipping log loss due to general error: {e}")
    ...
    if logs is not None and isinstance(logs, list) and len(logs) > 0:
        ...
        logs_df.to_csv(os.path.join(output_dir, "training_log.csv"), index=False)

    print(f"Results saved in: {output_dir}")



# === Resulting VGG11 with image + structured data ===
...
def save_run_outputs(model, X, y_true, y_pred, name, base_output_dir, use_timestamp_subfolder=True):
    ...
    try:
        ...
        if isinstance(X, tuple) and len(X) == 2:
            ...
            y_proba = None
            print("Skipping log loss: unknown input format.")

        # Log loss
        if y_proba is not None and y_proba.shape[0] == len(y_true_np):
            logloss = log_loss(y_true_np, y_proba)
            with open(os.path.join(output_dir, f"{name}_metrics.txt"), "a") as f:
                f.write(f"Log Loss: {logloss:.4f}\n")
        else:
            print("Skipping log loss: shape mismatch.")
